...
def decode_image(im_file, im_info):
    ...
    """
    # im = cv2.imdecode(im_file, 1)  # BGR mode, but need RGB mode
    # im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    im_info['im_shape'] = np.array(im_file.shape[:2], dtype=np.float32)
    im_info['scale_factor'] = np.array([1., 1.], dtype=np.float32)
    return im_file, im_info
...
class PadStride(object):
    """ padding image for models with FPN, instead PadBatch(pad_to_stride) in original config
    Args:
        stride (bool): models with FPN need image shape % stride == 0
    """
