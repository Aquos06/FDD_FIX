...
from pp_deploy.python.preprocess import preprocess, Resize, NormalizeImage, Permute, PadStride, LetterBoxResize, WarpAffine, Pad, decode_image
from pp_deploy.python.visualize import visualize_box_mask
from pp_deploy.python.utils import argsparser, Timer, get_current_memory_mb, multiclass_nms, coco_clsid2catid
...
class Detector(object):
    """
    Args:
        pred_config (object): config of models, defined by `Config(model_dir)`
        model_dir (str): root path of models.pdiparams, models.pdmodel and infer_cfg.yml
        ...
        trt_calib_mode (bool): If the models is produced by TRT offline quantitative
...
def create_inputs(imgs, im_info):
    """generate input for different models type
    Args:
        imgs (list(numpy)): list of images (np.ndarray)
        im_info (list(dict)): list of image info
    Returns:
        inputs (dict): input of models
    """
    inputs = {}
...
class PredictConfig:
    ...
    Args:
        model_dir (str): root path of models.yml
    """
    ...
    def __init__(self, model_dir):
        ...
        if self.arch == 'RCNN' and yml_conf.get('export_onnx', False):
            print(
                'The RCNN export models is used for ONNX and it only supports batch_size = 1'
            )
        # self.print_config()
    ...
    def check_model(self, yml_conf):
        """
        Raises:
            ValueError: loaded models not in supported models type
        """
...
def load_predictor(model_dir,
    ...
    infer_model = os.path.join(model_dir, 'models.pdmodel')
    infer_params = os.path.join(model_dir, 'models.pdiparams')
    if not os.path.exists(infer_model):
        infer_model = os.path.join(model_dir, 'inference.pdmodel')
        infer_params = os.path.join(model_dir, 'inference.pdiparams')
        if not os.path.exists(infer_model):
            raise ValueError(
                "Cannot find any inference models in dir: {},".format(model_dir))
