...
class Track(object):
    ...
    def __init__(self, prediction, trackIdCount, position):
    ...
    def Update(self, detections, boxes):
        ...
        if (len(self.tracks) == 0):
            for i in range(len(detections)):
                track = Track(detections[i], self.trackIdCount, boxes[i])
                self.trackIdCount += 1
        ...
        if len(del_tracks) > 0:  # only when skipped frame exceeds max
            for id in del_tracks[::-1]:
                if id < len(self.tracks):
                    self.delete_tracks_ids.append(self.tracks[id].track_id)
                    del self.tracks[id]
                    del assignment[id]
                else:
