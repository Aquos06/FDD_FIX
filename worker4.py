...
class Worker4(QObject):
    ...
    def openconfig(self):
        ...
        self.user = self.config_file['channel4']['user']
        self.ip = self.config_file['channel4']['ip']
        self.password = self.config_file['channel4']['password']
    ...
    def checkROI(self):
        ...
        if data['Camera4']['change'] == True:
            self.mask_person = cv2.imread('./ROI/Camera4/person.jpg')
            self.mask_fall = cv2.imread('./ROI/Camera4/fall_down.jpg')
            self.mask_PPE = cv2.imread('./ROI/Camera4/PPE.jpg')
            
            data['Camera4']['change'] = False
    ...
    def checkMin(self):
        if self.config_file['channel4']['ROI'] == True:
            ...
            self.minWidth = data['Camera4']['Min']['width']
            self.minHeight = data['Camera4']['Min']['height']
            self.maxWidth = data['Camera4']['Max']['width']
            self.maxHeight = data['Camera4']['Max']['height']
    ...
    def run(self):
        ...
        while True:
            ...
            if self.config_file['channel4']['change'] == True:
                self.openconfig()

            if self.ROI['Camera4']['Person'] == True:
                self.img = cv2.addWeighted(self.img,1,self.mask_person, 0.3,0)
            
            if self.ROI['Camera4']['FallDown'] == True:
                self.img = cv2.addWeighted(self.img,1, self.mask_fall,0.3,0)
            
            if self.ROI['Camera4']['PPE']['active'] == True:
                self.img = cv2.addWeighted(self.img, 1, self.mask_PPE, 0.3,0)
            
            if self.config_file['channel4']['active'] == False:
                self.noCamera.emit(True,4)
                self.totalfall.emit(0,0,4)

            elif self.ready== True and self.detect == True and self.config_file['channel2']['active'] == True:
                self.noCamera.emit(False,4)
                start = time.time()
                #print(type(self.img))	
                if self.ROI['Channel4']['active'] == True:
                    self.checkROI()
                ...
                person_coor = specific(person_coor)
                ...
                self.koordinat.emit(fall_coor,4)
                self.predictor.frame_id += 1

                filename, total_fall = self.findfall.final_fall(fall_coor,coor_track,self.img,4,self.ROI['Camera4']['active'],self.function['function']['light_delay']) #give self function
                self.img, total_peeps, self.total_people = drawBbox(self.img.copy(), coor_track, self.total_people,fall_coor)

                for file in filename:
                    self.fall.emit(file,4)
                try:
                    if self.zoom['Channel4'] == True:
                        self.zoom_on.emit(True,4)
                except:
                    pass

                self.totalfall.emit(total_fall,total_peeps,4)
